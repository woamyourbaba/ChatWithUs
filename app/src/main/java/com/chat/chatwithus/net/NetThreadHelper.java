package com.chat.chatwithus.net;
...
import com.chat.chatwithus.activity.BaseActivity;
...
public class NetThreadHelper implements Runnable{
    ...
    private int userCount = 0; //用户个数。

    private Queue<ChatMessage> receiveMsgQueue;	//消息队列,在没有聊天窗口时将接收的消息放到这个队列中
    private Vector<ReceiveMsgListener> listeners;	//ReceiveMsgListener容器，当一个聊天窗口打开时，将其加入。一定要记得适时将其移除，否则消息丢失
    ...
    public void noticeOnline(){	// 发送上线广播
        ...
        try {
            //broadcastAddr = InetAddress.getByName("224.0.0.1");
            broadcastAddr = InetAddress.getByName("255.255.255.255");	//广播地址
            System.out.println(broadcastAddr.isMulticastAddress());
